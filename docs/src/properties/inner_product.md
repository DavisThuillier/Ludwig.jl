...
Transport coefficients are expressed as matrix elements of ``L^{-1}`` under this inner product, i.e., ``\langle a | L^{-1} | b \rangle``, which are evaluated numerically by solving the linear system ``L \phi = b`` and then summing ``\sum_\mathbf{k} w_\mathbf{k} \, a_\mathbf{k} \, \phi_\mathbf{k}``. See [`inner_product`](@ref).
