...
# Fields
- `permutation::Vector{Int}`: permutation of `1:n` as a vector of images;
  `permutation[i]` is the image of `i`.
...
struct PermutationGroupElement <: GroupElement
    ...
    function PermutationGroupElement(perm)
        sort(perm) != collect(eachindex(perm)) &&
            throw(ArgumentError("Improper permutation."))
        return new(perm)
    end
end
...
function Base.show(io::IO, g::PermutationGroupElement)
    print(io, "PermutationGroupElement(", g.permutation, ")")
end

function Base.show(io::IO, ::MIME"text/plain", g::PermutationGroupElement)
    n = length(g)
    print(io, "PermutationGroupElement of length ", n, ":\n  ")
    print(io, "(", join(1:n, " "), ") -> (", join(g.permutation, " "), ")")
end

"""
    get_transposition(i, j, n)

Return the transposition in `Sₙ` that swaps `i` and `j` and fixes all other elements.
"""
...
"""
    get_identity_permutation(n)

Return the identity permutation of length `n` as a [`PermutationGroupElement`](@ref).
"""
...
Return the elements of the group generated by `generators` under multiplication.

The identity element is placed first in the returned vector. At least one generator must be
supplied.
"""
function closure(generators...)
    length(generators) == 0 && throw(ArgumentError("At least one generator must be passed."))
...
"""
    PermutationGroup(generators::PermutationGroupElement...)
...
# Fields
- `elements::Vector{PermutationGroupElement}`: full element list closed under
  multiplication, with the identity placed first.
...
Return the Cartesian coordinates of the `p`-th `n`-th root of unity as a 2-element vector.
"""
...
function get_matrix_representation(g::PermutationGroupElement)
    ...
    n < 2 && throw(DimensionMismatch(
        "Permutation must have length at least 2 to be represented by a 2x2 matrix."
    ))
