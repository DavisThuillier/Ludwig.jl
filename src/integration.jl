module Integration
    ...
    function Kabc!(ζ, u, a::Patch, b::Patch, c::Patch, d::SimplePatch, T::Real)
        δ = a.energy + b.energy - c.energy - d.energy # Energy conservation violations
    ...
    function electron_electron(grid::Vector{Patch}, f0s::Vector{Float64}, i::Int, j::Int, itps::Vector{ScaledInterpolation}, T::Real, Weff_squared, rlv, bz; umklapp = true)
        Lij::Float64 = 0.0
        w123::Float64 = 0.0
        w124::Float64 = 0.0
        ...
        for m in eachindex(grid)
            ...
            for μ in eachindex(itps)
                p = SimplePatch(
                    e = itps[μ](kijm_rlb[1], kijm_rlb[2]),
                    k = umklapp ? kijm : map_to_bz(kijm, bz, rlv, invrlv),
                    v = invrlv * Interpolations.gradient(itp, kijm_rlb[1], kijm_rlb[2]),
                    band_index = μ
                )
            ...
            for μ in eachindex(itps)
                p = SimplePatch(
                    e = itps[μ](qimj_rlb[1], qimj_rlb[2]),
                    k = umklapp ? kijm : map_to_bz(kijm, bz, rlv, invrlv),
                    v = Interpolations.gradient(itp, qimj_rlb[1], qimj_rlb[2]),
                    band_index = μ
                )
